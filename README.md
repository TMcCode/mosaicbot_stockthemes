...
**Supabase (optional):** To enable **Sign in** (Google, GitHub, magic link) and the `/my` watchlist area, set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` in `.env.local` (see `.env.local.example`). Optional `NEXT_PUBLIC_AUTH_OAUTH_PROVIDERS` (default `google,github`). Add the same **Variables** in GitHub Actions for production Pages. Enable providers in Supabase **Authentication → Providers** and allow **Redirect URLs** for `/auth/callback` (see `docs/AUTH_WATCHLIST_IMPLEMENTATION_PLAN.md`).
