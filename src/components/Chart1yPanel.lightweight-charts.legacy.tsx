/**
 * Archived TradingView Lightweight Charts implementation (same library family as TradingView).
 * Not used in production — Plotly is active (`ChartPlotlyInner`).
 *
 * Revert: in `Chart1yPanel.tsx`, replace the Plotly `dynamic(...)` block with:
 *
 ...
 *
 * (Remove Plotly `dynamic` import and `ChartPlotlyInner` usage.)
 */
"use client";
...
export type Chart1yPanelLightweightChartsLegacyProps = {
  chart1y: ThemeChart1yV0 | undefined;
};

/** @deprecated Archived — use Plotly via `Chart1yPanel` + `ChartPlotlyInner`. Kept for easy revert only. */
export function Chart1yPanelLightweightChartsLegacy({
  chart1y,
}: Chart1yPanelLightweightChartsLegacyProps) {
